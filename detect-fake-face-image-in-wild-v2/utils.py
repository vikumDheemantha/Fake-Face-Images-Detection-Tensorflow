...
def memory_data_batch(memory_data_dict, batch_size, preprocess_fns={}, shuffle=True, num_threads=16,
    ...
    with tf.compat.v1.name_scope(scope, 'memory_data_batch'):
        ...
        data_values = tf.compat.v1.train.slice_input_producer(list(tensor_dict.values()), shuffle=shuffle,
                                                              capacity=data_num)
...
def read_labeled_image_list_4_ilsvrc(image_list_file):
    ...
    filenames = []
    for filename in mydir:
        filename2 = image_list_file + filename
...
def disk_image_batch(image_paths, batch_size, shape, preprocess_fn=None, shuffle=True, num_threads=16,
    ...
    with tf.compat.v1.name_scope(scope, 'disk_image_batch'):
        ...
        image = tf.cast(tf.image.resize(image, [re_size, re_size], method=tf.image.ResizeMethod.BICUBIC),
                        dtype=tf.float32) / 127.5 - 1
