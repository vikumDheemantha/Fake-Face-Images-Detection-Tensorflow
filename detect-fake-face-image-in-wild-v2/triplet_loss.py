...
def _pairwise_distances(embeddings, squared=False):
    ...
    dot_product = tf.matmul(embeddings, tf.transpose(a=embeddings))
...
def _get_anchor_positive_triplet_mask(labels):
    ...
    indices_equal = tf.cast(tf.eye(tf.shape(input=labels)[0]), tf.bool)
...
def batch_all_triplet_loss(labels, embeddings, margin, squared=False):
    ...
    num_positive_triplets = tf.reduce_sum(input_tensor=valid_triplets)
    num_valid_triplets = tf.reduce_sum(input_tensor=mask)
    fraction_positive_triplets = num_positive_triplets / (num_valid_triplets + 1e-16)

    # Get final mean triplet loss over the positive valid triplets
    triplet_loss = tf.reduce_sum(input_tensor=triplet_loss) / (num_positive_triplets + 1e-16)
